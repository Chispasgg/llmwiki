      ...
      {isHosted && (
        <>
          ...
          <section>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Use API keys to connect MCP clients like Claude Desktop or Claude Code.
            </p>

            {showCreateForm && (
              <form onSubmit={handleCreateKey} className="mt-4 flex items-center gap-2">
                <input
                  type="text"
              ...
              <div className="mt-4 rounded-lg border border-border bg-muted/40 p-4 space-y-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium">{newKeyResult.name || 'API Key'}</p>
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-1.5">MCP config for this key:</p>
                  <div className="relative">
                    ...
                    >
                      {configCopied ? <><Check size={11} />Copied</> : <><Copy size={11} />Copy</>}
                    </button>
                  </div>
                </div>
            ...
            {keys.length > 0 && (
              <ul className="mt-4 divide-y divide-border rounded-lg border border-border overflow-hidden">
                {keys.map((k) => (
                  <li key={k.id} className="flex items-center gap-3 px-4 py-3 bg-background">
            ...
            {keys.length === 0 && !showCreateForm && !newKeyResult && (
              <p className="mt-4 text-sm text-muted-foreground">No API keys yet.</p>
            )}

            <p className="mt-3 text-xs text-muted-foreground">
              MCP URL:{' '}
              <code className="bg-muted px-1.5 py-0.5 rounded font-mono">{MCP_URL}</code>
            </p>
          </section>
        </>
      )}
